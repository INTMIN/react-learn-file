...
const OneDriveList = (props) => {
  ...
  const columns = [
    ...
    { title: "修改时间", dataIndex: "time", width: 400, key: "time" },
    {
      ...
      key: "x",
      render: (_, record) =>
        record.type !== "folder" && record.childCount > 0 ? <Link to={{pathname: `/list/${record.name}`}} >查看</Link>
        : <a href={`https://1197052014571378.cn-beijing.fc.aliyuncs.com/2016-08-15/proxy/henshangOneDrive/OneDriveApi/download?id=${record.id}`} target="_blank" rel="noopener noreferrer">点击下载</a>,
    },
  ];
